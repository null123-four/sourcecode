        ...
        <el-row style="padding: 10px;margin-left: 5px;">
            <el-row style="display: flex;justify-content: left;gap: 6px;">
                ...
                <el-select style="width: 160px;" @change="fetchFreshData" size="small"
                    v-model="cookbookNutrimentQueryDto.cookbookId" placeholder="食谱">
                    <el-option v-for="item in cookbookItems" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                </el-select>
                <el-select style="width: 160px;" @change="fetchFreshData" size="small"
                    v-model="cookbookNutrimentQueryDto.nutrimentId" placeholder="营养素">
        ...
        async fetchCookBookListItem() {
            try {
                ...
                if (response.data.code === 200) {
                    ...
                    const allCookbook = { id: null, name: '全部' };
                    // 头插
                    this.cookbookItems.unshift(allCookbook);
                }
            } catch (error) {
                console.log(error);
            }
        ...
        async fetchNutrimentListItem() {
            try {
                ...
                if (response.data.code === 200) {
                    ...
                    const allNutriment = { id: null, name: '全部' };
                    // 头插
                    this.nutrimentItems.unshift(allNutriment);
                }
            } catch (error) {
                console.log(error);
            }
